...
copilotNames <- "Vivian Zhang" # # insert the co-pilot's name here e.g., "Michael Frank". If there are multiple co-pilots enter both names in a character string e.g., "Tom Hardwicke, Bob Dylan"
...
copilotStartDate <- 10/19/2018 # insert the co-pilot's start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
...
The authors provided extensive code to reproduce the analyses and figures in the manuscript. The code was written in Matlab 2014, we conducted our reproducibility attempt in Matlab 2015 and copiloted in Matlab 2018.
...
COPILOT_MATLAB OUTPUT (Optional exclusions commented): in the high self-control group, the paths for taste and health were quite similar and became significantly greater than zero at approximately the same time (health: t = 72; taste: t = 66)
...
```{r}
## High self-control 
#Copilot with optional exclusion commented
# heathfulness
reportObject <- reproCheck(reportedValue = '67', obtainedValue = 72, valueType = 'mean')
#tastiness
reportObject <- reproCheck(reportedValue = '60', obtainedValue = 66, valueType = 'mean')
...
COPILOT_MATLAB OUTPUT (Optional exclusions commented): for the low self-control group, the latency at which taste became significant was similar to that of the high self-control group (t = 58), but that for health occurred much later (t = 88).
...
```{r}
## Low self-control
# Copilot with option exclusion commented
# heathfulness
reportObject <- reproCheck(reportedValue = '56', obtainedValue = 58, valueType = 'mean')
#tastiness
reportObject <- reproCheck(reportedValue = '85', obtainedValue = 88, valueType = 'mean')
```
...
COPILOT_MATLAB OUTPUT (Optional exclusions commented): The normalized time at which taste and health became significant did not differ for high self-control subjects (mean health: t = 74.30; mean taste: t = 69.00; mean difference = -6.80; t(9)=-1.67, p=0.06, one-tailed)
...
# Copilot with optional exclusion commented
# heathfulness
reportObject <- reproCheck(reportedValue = '69.50', obtainedValue = 74.30, valueType = 'mean')

# tastiness
reportObject <- reproCheck(reportedValue = '68.54', obtainedValue = 69.00, valueType = 'mean')

# t-test values
reportObject <- reproCheck(reportedValue = '9', obtainedValue = 9, valueType = 'df')
reportObject <- reproCheck(reportedValue = '-.46', obtainedValue = -1.67, valueType = 't')
reportObject <- reproCheck(reportedValue = '.33', obtainedValue = 0.06, valueType = 'p')
```
...
COPILOT_MATLAB OUPUT (Optional exclusions commented): but they were significantly different in the low self-control group (mean health: t = 84.50; mean taste: t = 67.35; mean difference = -15.70, t(9)=-2.84, p=0.01, one-tailed).

```{r}
## High self-control statistics
# heathfulness
reportObject <- reproCheck(reportedValue = '81.22', obtainedValue = 82.38, valueType = 'mean')

# tastiness
reportObject <- reproCheck(reportedValue = '61.35', obtainedValue = 63.65, valueType = 'mean')
...
# Copilot with optional exclusion commented
# heathfulness
reportObject <- reproCheck(reportedValue = '81.22', obtainedValue = 84.50, valueType = 'mean')

# tastiness
reportObject <- reproCheck(reportedValue = '61.35', obtainedValue = 67.35, valueType = 'mean')

# t-test values
reportObject <- reproCheck(reportedValue = '8', obtainedValue = 9, valueType = 'df')
reportObject <- reproCheck(reportedValue = '-2.5', obtainedValue = -2.84, valueType = 't')
reportObject <- reproCheck(reportedValue = '.02', obtainedValue = .01, valueType = 'p')
```
